...
// 监听合并任务完成后自动刷新已处理音频列表
watch(() => mergeTaskStore.status, async (val) => {
  if (val === 'completed' || val === 'failed' || val === 'cancelled') {
    mergeTaskStore.__stopPolling && mergeTaskStore.__stopPolling();
  }
  if (val === 'completed') {
    await nextTick();
    emit('process-success'); // 通知父组件刷新已处理音频列表
  }
});
...
const mergeSelectedFiles = async () => {
  ...
  mergeTaskStore.requestId = requestId;
  mergeTaskStore.status = 'processing';
  mergeTaskStore.progress = 0;
  mergeTaskStore.message = '正在提交处理任务...';
  ...
  try {
    ...
    // 启动轮询进度
    mergeTaskStore.__startPolling && mergeTaskStore.__startPolling();
  } catch (err) {
    mergeTaskStore.status = 'failed';
    mergeTaskStore.message = err.response?.data?.detail || '处理音频文件时出错';
    mergeTaskStore.show = true;
    mergeTaskStore.__stopPolling && mergeTaskStore.__stopPolling();
  }
};
